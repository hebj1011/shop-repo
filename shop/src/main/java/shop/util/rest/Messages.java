...
/**
 * @author <a href="mailto:Simon.Holzmayer@HS-Karlsruhe.de">Simon Holzmayer</a>
 */
@ApplicationScoped
@Log
public class Messages {
	...
	private static final List<Locale> LOCALES_DEFAULT = Arrays
			.asList(Locale.ENGLISH);
	...
	private transient Map<String, ResourceBundle> bundlesLanguageStr; // z.B.
																		// "en"
																		// als
																		// Schluessel
																		// auch
		...
		if (locales == null) {
			...
			for (String localeStr : localesIter) {
				try {
					...
					LOGGER.warnf("web.xml: %s ist kein gueltiger Sprachcode",
							localeStr);
					continue;
				}
	...
	public String getMessage(HttpHeaders headers, String key, Object... args) {
		final List<Locale> acceptableLocales = headers == null ? new ArrayList<Locale>(
				0) : headers.getAcceptableLanguages();
		final ResourceBundle bundle = getBundle(acceptableLocales);

		final String pattern = bundle.getString(key);
		final Locale locale = acceptableLocales == null
				|| acceptableLocales.isEmpty() ? Locale.getDefault()
